...
#Enable Logging
LogFile<-paste(baseWD,"\\Log\\",currentTime_fName,"_English_Short_Text_Mining_Scoring_v16.0_Log.txt",sep="")
zz_Log <- file(LogFile, open="wt")
sink(zz_Log, type = "message")

ListFile<-paste(baseWD,"\\Log\\",currentTime_fName,"_English_Short_Text_Mining_Scoring_v16.0_List.txt",sep="")
...
code_name<-paste(baseWD,"\\Code\\English_Short_Text_Mining_Scoring_v15.0.R",sep="")
code_description<-"English Short Text"
...
# Read Target Data
ProductionTable_English <- dbGetQuery(con, "select notification_number,notification_description,asset,operation,commodity,commodity_identifier,notification_datetime
...
n_docs_with_termDB<- dbGetQuery(con, "select * from naturallanguageprocessing.t_n_docs_with_term where language='English' and textlength='Short'")
...
save(correctListTab,file=paste(currentTime_fName,"_EN_SHORT__correctListTab.rda",sep=""))
}
...
setwd(paste(baseWD,"\\Backup",sep=""))
save(n_docs_with_termDB2,file=paste(currentTime_fName,"_EN_SHORT_n_docs_with_termDB2.rda",sep=""))
...
##################
setwd(paste(baseWD,"\\Interim",sep=""))
save(outputFinal7,file="outputFinal7_English_Short.rda")
...
#Delete
returnDelete2<-dbSendUpdate(con,"delete from naturallanguageprocessing.t_n_docs_with_term where language='English' and textlength='Short';")
...
dbSendUpdate(con,"update common_tables.t_error_log
			set
			 ...
			 where code_description='English Short Text'
			 and run_start_datetime=(select max(run_start_datetime) from common_tables.t_error_log where code_description='English Short Text')")
dbCommit(con)
}
